...
#define CS_VERSION    "_bonecrew_r1.2.5_2025_05_11884"
#ifndef CS_REVISION
#define CS_REVISION   "_bonecrew_"
#endif
#ifndef CS_GIT_VERSION
#define CS_GIT_VERSION "r1.2.0"
#endif
...
#if defined(__APPLE__) || defined(__FreeBSD__) || defined(__OpenBSD__)
#define NCAM_SIGNAL_WAKEUP     SIGCONT
#else
#define NCAM_SIGNAL_WAKEUP     SIGRTMAX-2
#endif
...
struct s_cardreader
{
	...
	int8_t          need_inverse;       // 0 = reader does inversing; 1 = inversing done by ncam
	//io_serial config
	int8_t          flush;
...
struct s_client
{
	...
	SIDTABS         sidtabs;
	SIDTABS         lb_sidtabs;
	int8_t          typ;                // first s_client is type s=starting (master) thread; type r = physical reader, type p = proxy reader both always have 1 s_reader struct allocated; type c = client (user logging in into ncam) type m = monitor type h = http server a = anticascader
	uint8_t         module_idx;
	uint16_t        last_srvid;
...
{
	uint32_t line; //linenr of ncam.whitelist file, starting with 1
...
{
	uint32_t line; //linenr of ncam.Cacheex file, starting with 1
	char type; // m
	uint16_t caid;
	uint32_t provid;
	uint16_t srvid;
...
struct s_config
{
	...
	char            *lb_savepath;                   // path where the stat file is save. Empty=default=/tmp/.ncam/stat
...
#include "ncam-log.h"
#include "ncam-log-reader.h"

// Add here *only* funcs that are implemented in ncam.c and are called in other places
...
// Until we find a better place for these (they are implemented in ncam-simples.h)
